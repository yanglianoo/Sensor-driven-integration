...
libmiddleware.so:$(SENSOR_DRIVER)/*.cpp $(SENSOR_MANAGE)/*.cpp $(UTILITY)/*.cpp
	$(CC) $(INCLUDE) $^ -fPIC -shared -o ./lib/$@ $(LIBPATH) $(LIB)  $(CFLAGS) `pkg-config --cflags --libs opencv4`


# 编译生成可执行文件
.PHONY:camera
camera:test_camera.cpp
	$(CC)  $(INCLUDE)  $< -o $@.out $(LIBPATH) $(LIB)  $(CFLAGS)   `pkg-config --cflags --libs opencv4`


.PHONY:laser
laser:test_laser.cpp
	$(CC)  $(INCLUDE)  $< -o $@.out $(LIBPATH) $(LIB)  $(CFLAGS) 
